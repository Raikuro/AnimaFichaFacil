...
export default function suite() {
  ...
  beforeEach(() => {
    this.sheetBuilder = Sheet.builder()
      ...
      .withPod(12)
      .withVol(15)
      .withClasses([new Clazz(GUERRERO, 1, new InvestedPdsForm(100, 100, 0, 100))])
  ...
  it('should set basic data correctly', () => {
    ...
    expect(sheet.classes).to.eql([new Clazz(GUERRERO, 1, new InvestedPdsForm(100, 100, 0, 100))]);
    ...
    expect(sheet.abilities.bonusHAtaque).to.equal(5)
    expect(sheet.abilities.bonusHParada).to.equal(5)
    expect(sheet.abilities.bonusLlevarArmadura).to.equal(5)

    expect(sheet.abilities.hAtaque).to.equal(45)
    expect(sheet.abilities.hParada).to.equal(45)
    expect(sheet.abilities.llevarArmadura).to.equal(50)
  })
  ...
  it('should full-update on changes on important values (i.e. race) Human to Sylvain', () => {
    ...
    expect(sheet.agiBonus).to.equal(-20);
    expect(sheet.conBonus).to.equal(-30);
    expect(sheet.desBonus).to.equal(-5);
    expect(sheet.fueBonus).to.equal(-10);
    expect(sheet.intBonus).to.equal(45);
    expect(sheet.perBonus).to.equal(10);
    expect(sheet.podBonus).to.equal(25);
    expect(sheet.volBonus).to.equal(30);
    ...
    expect(sheet.abilities.llevarArmadura).to.equal(45)
  })

  it('should full-update on changes on important values even if you do multiples times', () => {
    const sheet: Sheet = this.sheetBuilder
      .withRace(Race.HUMANO)
      .build();
